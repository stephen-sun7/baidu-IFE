...
class Calendar extends Component {
    ...
    handleInputClick() {
        const { calendar: { selected: { year, month, date }, display }, actions: { select } } = this.props;
        this.refs.input.value = ``;
        display ? display : select(year, month, date, 1);
    }
    handleConfirm() {
        const { calendar: { selected: { year, month, date } }, actions: { select } } = this.props;
        select(year, month, date, 0);
        this.refs.input.value = `${year}-${month}-${date}`;
    }
    ...
    handleCaptionClick() {
        ...
        display ^ 4 ? zoom(OUT, year, month, date, false) : display;
    }
    handleDataClick(i, j, isOutside) {
        ...
        return (event) => {
            switch (display) {
                ...
                case 2: zoom(IN, year, (i << 2) + j + 1, date, false); break;
                case 3: zoom(IN, year - year % 10 + (i << 2) + j - 1, month, date, isOutside(i, j)); break;
                case 4: zoom(IN, year - year % 100 + year % 10 + ((i << 2) + j - 1) * 10, month, date, isOutside(i, j)); break;
            }
        }
    }
                        ...
                        <tr key={i}>
                            {row.map((element, j) =>
                                <td
                                    ...
                                    onDoubleClick={this.handleConfirm}
                                >
                                    {element}
                                </td>
                            )}
